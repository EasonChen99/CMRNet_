...
def overlay_imgs(rgb, lidar, img_name):
    ...
    io.imsave(img_name, (blended_img*255).astype(np.uint8))
    return blended_img


def pose_error(rot1, trans1, rot2, trans2):
    """
    compute the rotation and translation error between two pose.
    Args:
        rot1, rot2 (torch.Tensor): rotation quaternion [4x1]
        trans1, rot2 (torch.Tensor): translation vector [3x1]
    Returns:
        rot_err (torch.Tensor): rotation error in degree
        trans_err (torch.Tensor): translation error in meter
    """
    quat_error = quaternion_distance(rot1.unsqueeze(0), rot2.unsqueeze(0), rot1.device)
    angle_distance = 180*quat_error/torch.tensor(np.pi)
    # inner_prod=rot1[0]*rot2[0]+rot1[1]*rot2[1]+rot1[2]*rot2[2]+rot1[3]*rot2[3]
    # cos_theta=2*inner_prod**2-1
    # angle_distance = 180.*torch.acos(cos_theta)/torch.tensor(np.pi)

    position_distance = torch.norm(trans1-trans2)
    return angle_distance, position_distance
