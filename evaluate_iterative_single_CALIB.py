...
def main(_config, seed):
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            depth_img += 1000.
            depth_img = visibility.depth_image(uv.contiguous(), depth.contiguous(), depth_img, uv.shape[0], real_shape[1], real_shape[0])
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                T_predicted, R_predicted = models[iteration].forward(rgb, lidar)
